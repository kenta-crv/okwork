...
class GptArticleGenerator
  ...
  def self.generate_body(column)
    ...
    if column.prompt.present? && column.prompt.length > 50
     Rails.logger.info("カスタムpromptを優先使用")

     custom_prompt = <<~PROMPT
      あなたはプロのWebライターです。
      以下の指示に忠実に従って、SEOを意識した記事本文を生成してください。
     ...
     response = call_gpt_api(custom_prompt)
     return response&.dig("choices", 0, "message", "content") || original_body
    end
  ...
  def self.service_profile(category)
    ...
    when "自販機"
      <<~TEXT
        ...
        サービス概要:
        自販機ねっとは、自動販売機を設置したい企業・個人に対して、
        1) メーカーによる無償設置が可能な場合は、そのメーカーを比較・選定できる一括見積サービスを提供し、
        2) 売上条件などの理由でメーカー設置ができない場合には、当社が独自に提供する自動販売機を設置できるようにするサービスである。
    
        メーカー自販機の実態:
        一般的に、自動販売機の設置はメーカー負担で行われ、設置費用は原則無償である。
        しかし現在は、物価高・原材料高騰・物流コスト増加などの影響により、
        各メーカーともに「十分な売上が見込めない場所」では採算が取れず、
        売上が低いエリアでは既存自販機の撤去が進んでいる。
        実務上、売上想定本数が月400本未満の場合、多くのメーカーでは新規設置がほぼ不可能な状況にある。
    
        かつては、山道・国道沿い・人通りの少ない場所などでも売上本数に関係なく設置されていたが、
        現在はそのような場所ほどメーカー撤去の対象になっており、
        「置きたいのに置けない」「以前はあった自販機が撤去された」というケースが増えている。
    
        自販機ねっとの提供価値:
        自販機ねっとでは、
        ・メーカー設置が可能な場合 → 無償設置前提で、複数メーカーを比較できる一括見積を提供
        ・メーカー設置ができない場合 → 当社が提供する自社自販機を設置可能
    
        当社提供の自動販売機の特徴:
        ・メーカーに依存しないため、売上条件に左右されにくい
        ・コカ・コーラ、サントリー、伊藤園などメーカーを問わず、自由に飲料構成を組める
        ・設置後も、ユーザー側で自由にドリンク内容を変更できる
        ・企業だけでなく個人でも設置が可能
        ・メーカー撤去が起きているような立地でも設置できる可能性がある
    
        つまり、自販機ねっとは
        「メーカー設置できる人には最適なメーカー選定を」
        「メーカー設置できない人には、自社自販機という現実的な選択肢を」
        の両方を提供するサービスである。
      TEXT
    when "営業代行"
      <<~TEXT
        ...
        サービス概要:
        アポ匠は、ターゲット選定、法人リスト制作、テレアポ代行、フォーム営業、商談代行、インサイドセールスまで、営業活動に必要な一連のプロセスを一括で代行するBtoB向けサービスです。
        営業担当者の不足や新規商談獲得に課題を抱える企業に向け、営業リソースを手離れよく外注できる環境を提供します。
    
        強み:
        - 営業プロセスを一気通貫で代行するため、依頼者は個別対応や管理負担を最小化できる
        - 独自のノウハウに基づくターゲット選定とリスト制作により、高い商談成約率を実現
        - テレアポ、フォーム営業、商談代行、インサイドセールスを組み合わせた最適なパッケージで、新規顧客開拓の効率化を支援
        - 営業リソースを丸ごと委託できるため、社内リソースをコア業務に集中させることが可能
    
        つまり、アポ匠は単なる営業代行ではなく、**営業活動全体を安心して外注できるワンストップパートナー**として機能します。
      TEXT
    else
